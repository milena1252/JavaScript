...
plot: "Luke Skywalker joins forces with a Jedi Knight, a cocky pilot, a Wookiee and two droids to save the galaxy from the Empire's world-destroying battle station, while alsoattempting to rescue Princess Leia from the mysterious Darth Vad",
...
writer: "Steve Kloves, J.K. Rowling",
actors: ["Daniel Radcliffe", "Emma Watson", "Rupert Grint"],
plot: "As Harry Potter begins his sixth year at Hogwarts, he discovers an old book marked as 'the property of the Half-Blood Prince' and begins to learn more about Lord Voldemort\'sdark past.",
...
"Richard Harris"],
plot: "An orphaned boy enrolls in a school of wizardry, where he learns the truth about himself, his family andthe terrible evil that haunts the magical world.",
country: "United Kingdom, United States",
...
// 3. Сортировать фильмы по рейтингу по убыванию
const sortedFilms = films.map(film => film).sort ((a,b) => b.imdbRating - a.imdbRating);
...
// результатом этой функции должен быть отфильтрованный массив, с фильмами
// где число равно году выхода фильма.
function filmsByYear (films, year) {
...
// результатом этой функции должен быть новый отфильтрованный массив, с
// фильмами, где строка входит в название фильма.
function filmsByTitle (films, str) {
    return films.reduce ((acc, film) => {
...
console.log(filmsByTitle(films, 'potter'));
console.log(filmsByTitle(films, 'Star'));

// 7. Создать функцию, которая бы принимала массив фильмов и строку. А
// результатом этой функции должен быть отфильтрованный массив, с фильмами
// где строка входит в название фильма или в его сюжет.
function filmsByTitlePlot (films, str) {
    return films.reduce ((acc, film) => {
        if (film.title.toLowerCase().includes(str.toLowerCase()) || film.plot.toLowerCase().includes(str.toLowerCase())) {
            acc.push(film);
        }
    return acc;
    }, []);
...
console.log(filmsByTitlePlot(films, 'family'));

// 8. Создать функцию, которая бы принимала 3 параметра: 1)массив фильмов , 2)
// строка(название поля, например 'title') и строку/число(значение поля C). А результатом этой функции должен быть отфильтрованный массив,
// где параметры 2 и 3 равны в объекте фильма. Например: передаем (films, 'title',
// 'Black Widow') и на выходе получаем фильм с id=1 если передаем (films, 'year',
// 2011) , то получаем фильм с id=2
function filmsByField (films, fieldName, fieldValue) {
    return films.reduce ((acc, film) => {
        if (film[fieldName] === fieldValue) {
            acc.push(film);
        }
    return acc;
    }, []);
...
console.log(filmsByField(films, 'title', 'Black Widow'));
console.log(filmsByField(films, 'year', 2011));
